...
pub struct PriceForOverConsumingRepository<'a> {
    client: &'a mut tokio_postgres::Client,
}

impl<'a> PriceForOverConsumingRepository<'a> {
    pub fn new(client: &'a mut tokio_postgres::Client) -> Self {
        PriceForOverConsumingRepository { client }
    }

    pub async fn create(&mut self, price_for_over_consuming: &PriceForOverConsuming) -> Result<(), Error> {
        self.client
            .execute(
    ...
    pub async fn read_all(&mut self) -> Result<Vec<PriceForOverConsuming>, Error> {
        let rows = self
            .client
            .query("SELECT id, voltage_level_id, price_category_id, power_level_id, contract_type_id, price, year, month FROM price_for_over_consuming", &[])
            .await?;
        let mut results = Vec::new();
        for row in rows {
            results.push(PriceForOverConsuming {
                id: row.get(0),
    ...
    pub async fn update(&mut self, price_for_over_consuming: &PriceForOverConsuming) -> Result<(), Error> {
        self.client
            .execute(
                "UPDATE price_for_over_consuming SET price = $1 WHERE id = $2",
                &[
                    &price_for_over_consuming.price,
                    &price_for_over_consuming.id,
                ],
            )
            .await?;
        Ok(())
    ...
    pub async fn delete(&mut self, id: i32) -> Result<(), Error> {
        self.client
            .execute("DELETE FROM price_for_over_consuming WHERE id = $1", &[&id])
            .await?;
        Ok(())
    }

...
pub struct PeakTariffRepository<'a> {
    client: &'a mut tokio_postgres::Client,
}

impl<'a> PeakTariffRepository<'a> {
    pub fn new(client: &'a mut tokio_postgres::Client) -> Self {
        PeakTariffRepository { client }
    }

    pub async fn create(&mut self, peak_tariff: &PeakTariff) -> Result<(), Error> {
        self.client
            .execute(
    ...
    pub async fn read_all(&mut self) -> Result<Vec<PeakTariff>, Error> {
        let rows = self
            .client
            .query("SELECT id, voltage_level_id, power_level_id, contract_type_id, price, year, month FROM peak_tariff", &[])
            .await?;
        let mut results = Vec::new();
        for row in rows {
            results.push(PeakTariff {
                id: row.get(0),
    ...
    pub async fn update(&mut self, peak_tariff: &PeakTariff) -> Result<(), Error> {
        self.client
            .execute(
    ...
    pub async fn delete(&mut self, id: i32) -> Result<(), Error> {
        self.client
            .execute("DELETE FROM peak_tariff WHERE id = $1", &[&id])
            .await?;
        Ok(())
    }

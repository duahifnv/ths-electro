...
pub struct HourTariffRepository<'a> {
    client: &'a mut tokio_postgres::Client,
}

impl<'a> HourTariffRepository<'a> {
    pub fn new(client: &'a mut tokio_postgres::Client) -> Self {
        HourTariffRepository { client }
    }
    
    pub async fn create(&mut self, records: &[HourTariff]) -> Result<(), Error> {
    ...
    pub async fn update(&mut self, records: &[HourTariff]) -> Result<(), Error> {
        let transaction = self.client.transaction().await?;
        for record in records {
            transaction
                .execute(

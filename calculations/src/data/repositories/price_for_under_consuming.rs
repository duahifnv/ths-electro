...
pub async fn create(
    ...
    price_for_under_consuming: &PriceForUnderConsuming,
) -> Result<(), Error> {
    conn
            .execute(
...
pub async fn read_by_params(
    ...
    year: u32,
    month: u8,
) -> Result<PriceForUnderConsuming, Error> {
    let row = conn
            .query_one("SELECT id, voltage_level_id, price_category_id, power_level_id, contract_type_id, price, year, month FROM price_for_under_consuming WHERE voltage_level_id = $1 AND price_category_id = $2 AND power_level_id = $3 AND contract_type_id = $4 AND year = $5 AND month = $6)", &[&voltage_level_id, &price_category_id, &power_level_id, &contract_type_id, &(year as i32), &(month as i16)],)
            .await?;
    Ok(PriceForUnderConsuming {
        id: row.get(0),
...
pub async fn update(
    conn: &mut PooledConnection<'_, PostgresConnectionManager<NoTls>>,
    price_for_under_consuming: &PriceForUnderConsuming, id: i32
) -> Result<(), Error> {
    conn.execute(
...
pub async fn delete(
    ...
    conn.execute(
        "DELETE FROM price_for_under_consuming WHERE id = $1",
        &[&id],
    )
    .await?;
    Ok(())

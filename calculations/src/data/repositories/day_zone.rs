...
pub struct DayZoneRepository<'a> {
    client: &'a mut tokio_postgres::Client,
}

impl<'a> DayZoneRepository<'a> {
    pub fn new(client: &'a mut tokio_postgres::Client) -> Self {
        DayZoneRepository { client }
    }
    
    pub async fn create(&mut self, records: &[DayZone]) -> Result<(), Error> {
    ...
    pub async fn update(&mut self, records: &[DayZone]) -> Result<(), Error> {
        let transaction = self.client.transaction().await?;
        for record in records {
            transaction
                .execute(
    ...
    pub async fn exists_by_id(&mut self, month: u8) -> Result<bool, Error> {

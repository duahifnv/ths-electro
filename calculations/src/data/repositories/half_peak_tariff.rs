...
pub struct HalfPeakTariffRepository<'a> {
    client: &'a mut tokio_postgres::Client,
}

impl<'a> HalfPeakTariffRepository<'a> {
    pub fn new(client: &'a mut tokio_postgres::Client) -> Self {
        HalfPeakTariffRepository { client }
    }

    pub async fn create(&mut self, half_peak_tariff: &HalfPeakTariff) -> Result<(), Error> {
        self.client
            .execute(
    ...
    pub async fn read_all(&mut self) -> Result<Vec<HalfPeakTariff>, Error> {
        let rows = self
            .client
            .query("SELECT id, voltage_level_id, power_level_id, contract_type_id, price, year, month FROM half_peak_tariff", &[])
            .await?;
        let mut results = Vec::new();
        for row in rows {
            results.push(HalfPeakTariff {
                id: row.get(0),
    ...
    pub async fn update(&mut self, half_peak_tariff: &HalfPeakTariff) -> Result<(), Error> {
        self.client
            .execute(
                "UPDATE half_peak_tariff SET price = $1 WHERE id = $2",
                &[
                    &half_peak_tariff.price,
                    &half_peak_tariff.id,
                ],
            )
            .await?;
    ...
    pub async fn delete(&mut self, id: i32) -> Result<(), Error> {
        self.client
            .execute("DELETE FROM half_peak_tariff WHERE id = $1", &[&id])
            .await?;
        Ok(())
    }

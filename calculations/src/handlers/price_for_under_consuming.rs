...
use crate::{
    ...
    dtos::price_for_under_consuming,
    mappers,
    queries::PriceForUnderConsumingQuery,
};

pub async fn create(
    State(pool): State<ConnectionPool>,
    Json(dto): Json<price_for_under_consuming::CreateDto>,
    ...
    repositories::price_for_under_consuming::create(
        &mut conn,
        &mappers::price_for_under_consuming::create_dto_to_model(&dto),
    )
    .await
    .unwrap();
    Ok(StatusCode::OK)
...
pub async fn get_by_params(
    State(pool): State<ConnectionPool>,
    Query(query): Query<PriceForUnderConsumingQuery>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    let mut conn = pool
        .get()
        .await
    ...
    let results = repositories::price_for_under_consuming::read_by_params(
        &mut conn,
...
pub async fn delete(
    ...
    let exists = repositories::price_for_under_consuming::exists_by_id(&mut conn, id)
        .await
        .unwrap();
    if !exists {
        return Err((
    ...
    repositories::price_for_under_consuming::delete(&mut conn, id)
        .await
        .unwrap();
    Ok(StatusCode::OK)
}
...
pub async fn update(
    State(pool): State<ConnectionPool>,
    Path(id): Path<i32>,
    Json(dto): Json<price_for_under_consuming::UpdateDto>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    let mut conn = pool
        .get()
        .await
    ...
    repositories::price_for_under_consuming::update(
        &mut conn,
        &mappers::price_for_under_consuming::update_dto_to_model(&dto),
        id
    )
    .await
    .unwrap();

...
use crate::{
    ...
    dtos::peak_tariff,
    mappers,
    queries::PeakTariffQuery,
};

pub async fn create(
    State(pool): State<ConnectionPool>,
    Json(dto): Json<peak_tariff::CreateDto>,
    ...
    repositories::peak_tariff::create(&mut conn, &mappers::peak_tariff::create_dto_to_model(&dto))
        .await
        .unwrap();
    Ok(StatusCode::OK)
}
...
pub async fn get_by_params(
    State(pool): State<ConnectionPool>,
    Query(query): Query<PeakTariffQuery>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    let mut conn = pool
        .get()
        .await
    ...
    let results = repositories::peak_tariff::read_by_params(
        &mut conn,
...
pub async fn delete(
    ...
    let exists = repositories::peak_tariff::exists_by_id(&mut conn, id)
        .await
        .unwrap();
    if !exists {
        return Err((
    ...
    repositories::peak_tariff::delete(&mut conn, id)
        .await
        .unwrap();
    Ok(StatusCode::OK)
}
...
pub async fn update(
    State(pool): State<ConnectionPool>,
    Path(id): Path<i32>,
    Json(dto): Json<peak_tariff::UpdateDto>,
) -> Result<impl IntoResponse, (StatusCode, String)> {
    let mut conn = pool
        .get()
        .await
    ...
    repositories::peak_tariff::update(
        &mut conn,
        &mappers::peak_tariff::update_dto_to_model(&dto),
        id,
    )
    .await
    .unwrap();
